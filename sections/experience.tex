%-------------------------------------------------------------------------------
%	SECTION TITLE
%-------------------------------------------------------------------------------
\cvsection{Work Experience}


%-------------------------------------------------------------------------------
%	CONTENT
%-------------------------------------------------------------------------------
\begin{cventries}

%---------------------------------------------------------
  \cventry
    {Senior Engineer} % Job title
    {Spotify Technology S.A.} % Organization
    {New York City, NY} % Location
    {Apr. 2022 - Present} % Date(s)
    {
      \begin{cvitems} 
        \item {Work on core ads team supporting over \$1.8B in ad sales}
        \item {Maintain over 30 backend services, tools, and libraries (Apex, Java, Python, GCP, gRPC, REST, Apex, Kubernetes)}
        \item {Designed and implemented the POC for a migration away from Google Ad Manager to our internal ad server during Hack Week 2024---this turned into a major initiative invested in heavily by the company at large, saving millions in annual contract fees}
        \item {Manage CI/CD pipeline and developer tooling---enhanced productivity by 20\% with Salesforce Chrome extension built during Hack Week 2023}
        \item {Undertook major migration from JavaScript to TypeScript in the OMS, reducing the need for hotfixes by 10\%}
        \item {Wrote Java service to directly update live orders in Google Ad Manager to resolve major incident, saving \$4.4M in potential makegoods}
        \item {Built the capability for serving combined SAI/DAI podcast ads in the OMS, increasing Spotify Audience Network (SPAN) revenue by 5\%}
        \item {Implemented the primary backend changes for Unified Ad Accounts project, leading to a 95\% reduction in duplicate ad account data in Salesforce}
        \item {Architect and clearly communicate complex technical solutions to stakeholders and cross-functional teams}
        \item {Mentor junior developers and maintain rigorous code quality standards}
      \end{cvitems}
    }

%---------------------------------------------------------
  \cventry
    {Founder \& CEO} % Job title
    {Forcecode} % Organization
    {New York City, NY} % Location
    {Nov. 2021 - Present} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Founded Forcecode, helping developers to learn Apex and prepare for technical interviews through coding challenges, tutorials, and more}
        \item {Build and maintain frontend and backend codebases, all servers, and all coding challenges (React, Next.js, Node.js, Express, TypeScript, PostgreSQL, Redis, GraphQL)}
        \item {Support over 1400 users}
        \item {Maintain and develop a Python library and CLI for managing 30+ Forcecode challenges in a custom CMS}
        \item {Designed and implemented Forcecode's proprietary ``managed package" to allow complex but scalable validation of coding challenges}
      \end{cvitems}
    }

%---------------------------------------------------------
  % \cventry -- example with no body
  %   {Site Reliability Engineer} % Job title
  %   {Danggeun Market Inc.} % Organization
  %   {Seoul, S.Korea} % Location
  %   {Feb. 2021 - Mar. 2021} % Date(s)
  %   {
  %   }

%---------------------------------------------------------
  \cventry
    {Principal/Senior Engineer} % Job title
    {Slalom, Inc.} % Organization
    {Denver, CO} % Location
    {Apr. 2021 - Apr. 2022} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Core developer on the Global Digital Engagement team at Slalom, regularly managing 15+ clients at a time}
        \item {Architected, built, and delivered several large-scale (>\$1M) projects for clients across multiple verticals}
        \item {Designed and implemented complex solutions across varied tech stacks (TypeScript, Python, Apex, JavaScript, REST, HTML)}
        \item {Led several phases of the development of Robin, an award-winning chatbot for the State of Connecticut (see \href{https://portal.ct.gov/das/dasblog/das-team-wins-government-experience-award}{\color{awesome-skyblue}here})}
      \end{cvitems}
    }

%---------------------------------------------------------
  \cventry
    {Software Engineer} % Job title
    {WOW! Internet, Cable, \& Phone} % Organization
    {Denver, CO} % Location
    {Nov. 2020 - Apr. 2021} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {In-house developer managing applications with 2,000+ daily active employees}
        \item {Wrote a set of coding standards and implemented a code review system for a team of 10+ offshore developers}
        \item {Responsible for overseeing the GitHub monorepo and reviewing/merging 5-10 pull requests per day}
        \item {Built a custom sales approval process to reduce approval times by 30\%}
        \item {Primary maintainer of multiple SOAP and REST-based integrations}
        \item {Created a sandbox refresh strategy to maintain data and code integrity between environments}
      \end{cvitems}
    }

%---------------------------------------------------------
  \cventry
    {Developer} % Job title
    {Youth Leadership Development Program} % Organization
    {Remote} % Location
    {May 2020 - Nov. 2020} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Implemented a frontend integration with Salesforce, leading to a 20\% increase in lead acquisition}
        \item {Built UI and client-side logic using HTML, JavaScript, jQuery, and CSS}
        \item {Collaborated with project manager and on/off-shore teams utilizing Agile methodologies}
        \item {Integrated the frontend with Google Maps JavaScript API, Geocoding API, and Places API}
      \end{cvitems}
    }

%---------------------------------------------------------
  \cventry
    {Freelance} % Job title
    {Web Developer} % Organization
    {Remote} % Location
    {Sep. 2017 - May 2020} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Completed end-to-end projects for clients in multiple verticals}
        \item {Worked primarily with React, JavaScript, HTML, and Node.js as a full-stack developer}
      \end{cvitems}
    }

%---------------------------------------------------------
\end{cventries}
